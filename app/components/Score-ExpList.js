	...
	if(allData && content){
		...
		scoringPlays.forEach((play, index) => (
			scoringPlaysDescription.push({
				...
				highlight: milestones[index].highlight.playbacks[3].url !== null ? milestones[index].highlight.playbacks[3].url : null
				})
		))
		...
		return scoringPlaysDescription;
	} else {
		return null;
	}
}
